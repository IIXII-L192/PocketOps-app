...
object DiagnosticLogger {
    ...
    private fun generateLogFile(context: Context, throwable: Throwable?): File? {
        try {
            ...
            val fileName = "PocketOps_log_${SimpleDateFormat("ddMMyyyy_HHmm", Locale.getDefault()).format(Date())}.json"
    ...
    fun getPendingCrashLogFile(context: Context): File? {
        ...
        if (file.exists()) {
            try {
                val timestampName = "PocketOps_log_${SimpleDateFormat("ddMMyyyy_HHmm", Locale.getDefault()).format(Date())}.json"
